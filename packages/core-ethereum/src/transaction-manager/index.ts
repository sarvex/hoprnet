...
class TransactionManager {
    ...
    private async buildTransaction(_payload: TransactionPayload, _nonce: NonceTypeAtTransactionCreation): Promise<Omit<Transaction, "status" | "broadcastedAt">> {
        // gets the latest nonce.
        const nonce = typeof _nonce === 'number' ? _nonce : (await this.nonceTracker.getNonceLock(this.fromAddress)).nextNonce
